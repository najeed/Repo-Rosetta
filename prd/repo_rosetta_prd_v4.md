...
docker run reporosetta/server \
  ...
  --llm-provider openai \   # or anthropic, azure-openai, local
  --storage postgres \
  --port 3000
```

The self-hosted server exposes the same web UI and API as the hosted version.
...
```bash
# Analyze a local repo (no network calls for code)
rosetta analyze ./my-private-repo --output ./docs/rosetta

# Analyze via GitHub token (code fetched locally)
rosetta analyze github.com/org/repo \
  --token $GITHUB_TOKEN \
  --output ./docs/rosetta \
  --format markdown
```
...
### 10.5 LLM Provider Options for Private Repos

For organizations that cannot send code-derived data to third-party LLM APIs, Repo Rosetta supports configurable LLM backends in self-hosted mode:

| Provider | Mode | Notes |
|---|---|---|
| Anthropic Claude | Hosted + Self-hosted | Default; strong code reasoning |
| OpenAI GPT-4 | Hosted + Self-hosted | Alternative |
| Azure OpenAI | Self-hosted | Data residency within Azure tenant |
| AWS Bedrock | Self-hosted | Data residency within AWS account |
| Local model (Ollama) | Self-hosted / CLI only | Zero external API calls; quality varies by model size |

The LLM provider is configurable per organization in self-hosted mode. Only the relevant code excerpts and graph metadata — never the full raw source — are sent to the LLM.
    ...
    steps:
      ...
      - uses: reporosetta/action@v1
        with:
          ...
          llm-provider: anthropic          # or: openai, azure-openai
...
| 6 | Configurable LLM backend (Azure OpenAI, Bedrock, Ollama) |
